...
export class StoreDetail implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.cardId = history.state?.cardId;
    if (this.cardId) {
    ...
    this.loyaltyService.updateLastAccess(this.cardId).subscribe({
      next: () => console.log('Último acceso actualizado'),
      error: (err) => console.error('Error al actualizar último acceso', err)
    })
  }
  ...
  openQrModal() {
    this.generateQrPayload(); // Generamos el contenido justo antes de abrir
    this.showQrModal = true;

    // Bloqueamos el scroll del fondo para una mejor UX
  ...
  private generateQrPayload() {
    const data = {
      c: this.cardId,           // Card ID
      s: this.store?.id,        // Store ID
      u: this.currentUserId,    // User ID
      t: Date.now()             // Timestamp (evita fraudes con capturas de pantalla viejas)
    };

    // Convertimos el JSON a una cadena y luego a Base64
    // Esto hace que el QR no sea legible por cualquier lector genérico
    const jsonString = JSON.stringify(data);
    this.qrPayload = btoa(jsonString);

    console.log('Payload generado:', this.qrPayload);
  }
}
